...
fun message(context: Context) {
    ...
    val action =
        NotificationCompat.Action.Builder(R.drawable.ic_launcher_foreground, "Reply", pendingIntent)
            .addRemoteInput(replyInput)

    val person = Person.Builder().setName("Aniruddha").build()
    val style =
        NotificationCompat.MessagingStyle(person).setConversationTitle("conversation").addMessage(
            "text 1",
    ...
    val notification = NotificationCompat.Builder(context, DEFAULT_ID)
        ...
        .setContentText("description").setStyle(style).addAction(action.build()).build()
...
class MessageReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context?, intent: Intent?) {
        RemoteInput.getResultsFromIntent(intent!!)?.getCharSequence(REPLY).let { text ->
            ...
            val person = Person.Builder().setName("Aniruddha").build()
            val person2 = Person.Builder().setName("VOdaaaa").build()
            val style =
                NotificationCompat.MessagingStyle(person).setConversationTitle("conversation")
                    .addMessage(
                        "text 1",
                        System.currentTimeMillis(), person
                    ).addMessage("text 2", System.currentTimeMillis(), person)
